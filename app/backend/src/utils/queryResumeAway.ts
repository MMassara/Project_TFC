...
(SUM(t2.away_team_goals - t2.home_team_goals)) AS goalsBalance,
ROUND(((SUM(CASE WHEN t2.home_team_goals < t2.away_team_goals THEN 3 ELSE 0 END) 
+ SUM(CASE WHEN t2.home_team_goals = t2.away_team_goals THEN 1 ELSE 0 END)) 
/ (SUM(CASE WHEN t2.away_team_id = t1.id THEN 1 ELSE 0 END) * 3)) * 100, 2) AS efficiency
FROM 
...
export default queryResumeAway;
